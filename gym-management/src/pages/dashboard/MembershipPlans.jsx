...
function computePrice(baseMonthly, cycle, coupon, allOffers) {
  ...
  let discount = 0;
  if (coupon) {
    const offer = allOffers.find(o => o.code === coupon.toUpperCase() && o.status === 'Active');
    if (offer) {
      discount = offer.type === 'percentage' ? Math.round(total * offer.value / 100) : offer.value;
    }
  }
...
export default function MembershipPlans() {
  ...
  const handleApplyCoupon = () => {
    ...
    if (found) {
      ...
      setCouponMsg(`✓ "${found.name}" applied — ${found.type === 'percentage' ? `${found.value}% off` : `₹${found.value} off`}`);
      setCouponOk(true);
    } else {
      setAppliedCoupon('');
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        {membershipPlans.map(plan => {
          ...
          const pricing = activeCycle.id !== 'custom'
            ? computePrice(baseMonthly, activeCycle, appliedCoupon, offers)
            : null;
