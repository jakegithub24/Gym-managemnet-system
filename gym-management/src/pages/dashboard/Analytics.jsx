    ...
    <div className="flex items-end justify-between gap-2" style={{ height }}>
      {data.map((item, i) => (
        <div key={i} className="flex-1 flex flex-col items-center gap-1">
          <div className="w-full bg-gray-800 rounded-t-lg relative" style={{ height: `${(item[dataKey] / maxVal) * 100}%`, minHeight: '4px', backgroundColor: color, opacity: 0.8 }} />
          ...
          <div className="space-y-3">
            {productSalesData.map(p => (
              <div key={p.product} className="flex items-center gap-3">
                ...
                <div className="w-20 flex-shrink-0 text-right"><p className="text-xs font-bold text-[#39FF14]">{fmtINR(p.revenue)}</p></div>
              </div>
            ))}
          </div>
          <div className="mt-4 pt-4 border-t border-gray-800 text-center">
            <p className="text-xs text-gray-500">Total Product Revenue</p>
            <p className="text-xl font-black text-[#39FF14]">{fmtINR(productRevenue)}</p>
          </div>
        </div>
